...
[ ] User Interaction Flow is a strictly formatted Mermaid flowchart (`flowchart TD`)
...
- **Mermaid strictness**: All `User Interaction Flow` blocks MUST use standard ````mermaid` `flowchart TD` syntax. Nodes MUST be quoted (e.g. `A(["Text"])`), edge labels MUST NOT contain parentheses, and you MUST NOT use HTML tags like `<br>`.
